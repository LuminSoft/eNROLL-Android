...
fun MailAuthUpdateScreenContent(
    ...
    val validateOtpMailAuthUpdateUseCase =
        ValidateOtpMailAuthUpdateUseCase(koinInject())

    val mailAuthUpdateSendOTPUseCase =
        MailAuthUpdateSendOTPUseCase(koinInject())

    val mailViewModel =
        remember {
            MailAuthUpdateViewModel(
                mailAuthSendOTPUseCase = mailAuthUpdateSendOTPUseCase,
                validateOtpMailAuthUseCase = validateOtpMailAuthUpdateUseCase
            )
        }

    mailViewModel.setStepUpdateId(updateViewModel.updateStepId.value!!)
