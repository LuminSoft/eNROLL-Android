...
class MainSignContractRepositoryImplementation(private val mainRemoteDataSource: MainSignContractRemoteDataSource) :
    ...
    override suspend fun getSignContractSteps(): Either<SdkFailure, StepSignContractModel> {
        return when (val response = mainRemoteDataSource.getSignContractSteps()) {
            is BaseResponse.Success -> {
                Either.Right(response.data as StepSignContractModel)
            }
    ...
    override suspend fun getSignContractFiles(): Either<SdkFailure, List<SignContractFileItemModel>> {
        return when (val response = mainRemoteDataSource.getSignContractFiles()) {
            is BaseResponse.Success -> {
                Either.Right(response.data as List<SignContractFileItemModel>)
            }
